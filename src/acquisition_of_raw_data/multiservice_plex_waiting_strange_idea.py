...
class AsyncWaitForService(object):

    def __init__(self, service_name, timeout=None):
        """Create an asynchronous wait_for_service."""
    ...
    def __call__(self, *args, **kwargs):
        ...
        fut = self.executor.submit(rospy.wait_for_service, self._service_name, *args, timeout= self._timeout, **kwargs)
...
class MultiServiceCaller:
    ...
    def __init__(self, list_services_, srvMsgType = Empty(), srvMsgTypeResponse = EmptyResponse(), wait_to_start=False, wait_for_responses = False, timeout=rospy.Duration(1)):
        ...
        for a_srv_name in list_services_:
            self.list_of_services.update({a_srv_name:[AsyncServiceProxy(a_srv_name, srvMsgType ),AsyncWaitForService(a_srv_name,timeout)]})
    ...
    def __call__(self, srvMsg):
        ...
        if self.wait_to_start: 
            responses = []
            for a_srv_name, (a_srv, a_wait_for_srv )in self.list_of_services.items():
                try:
                    rospy.loginfo(f"waiting for service {a_srv_name}")
                    a_response = a_wait_for_srv()
                    responses.append((a_srv_name,a_response))
                except rospy.ServiceException as e:
                    rospy.logerr(f"failed while waiting for srv?? [{a_srv_name}].")
                    self.error_list.append(f"Error: {e}")
            for a_srv_name, a_response in responses:
                try:
                    ...
                    rospy.logerr(f"failed while getting response from waiting for srv?? [{a_srv_name}].")
                    self.error_list.append(f"Error: {e}")
        responses = []
        for a_srv_name, (a_srv, a_wait_for_srv )in self.list_of_services.items():
            try:
        ...
        if self.wait_for_responses:
            for a_srv_name, a_response in responses:
                try:
                    ...
                    rospy.logerr(f"failed while getting response for srv?? [{a_srv_name}].")
                    self.error_list.append(f"Error: {e}")
        rospy.loginfo("Done.")
